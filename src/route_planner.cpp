...
RoutePlanner::RoutePlanner(RouteModel &model, float start_x, float start_y, float end_x, float end_y): m_Model(model) {
    // m_Model(model) is initialized using an initializer list
...
float RoutePlanner::CalculateHValue(RouteModel::Node const *node) {
    RouteModel::Node current_node = *node;
...
std::vector<RouteModel::Node> RoutePlanner::ConstructFinalPath(RouteModel::Node *current_node) {
    ...
    auto current_node_update = current_node;

    while(current_node_update != start_node){
        path_found.push_back(*current_node_update);
        distance = distance + current_node_update->distance(*current_node_update->parent);
        current_node_update = current_node_update->parent;
    }
